...
STOCK_NAMES = {
    ...
    "2059.TW": "川湖", "2327.TW": "國巨", "2330.TW": "台積電", "2481.TW": "強茂",
    "2882.TW": "國泰金", "2891.TW": "中信金", "3081.TWO": "聯亞", "4979.TWO": "華星光",
    "2317.TW": "鴻海", "2454.TW": "聯發科", "2603.TW": "長榮", "3231.TW": "緯創"
}
DEFAULT_TICKERS = ", ".join(STOCK_NAMES.keys())

st.sidebar.header("選股池與回測設定")
ticker_input = st.sidebar.text_area("股票代號（逗號分隔）", DEFAULT_TICKERS, height=150)
...
top_n = st.sidebar.slider("顯示標的數", 3, 30, 10)
...
if st.button("執行週期分析與選股", type="primary"):
    tickers = list(dict.fromkeys([x.strip().upper() for x in ticker_input.split(",") if x.strip()]))
    if "0050.TW" not in tickers:
        tickers.insert(0, "0050.TW")
    ...
    with st.spinner("下載資料、建立 500+ 組策略並執行時間切割驗證..."):
        ...
        for tab, (label, horizon) in zip(tabs, HORIZONS.items()):
            with tab:
                best = choose_strategy(frames, horizon, bench_filter, cost)
                if best is None:
                ...
                picks = rank_today(frames, s, horizon, bench_filter)
                if picks.empty:
...
- 「最佳」只代表所選股票池、期間、成本與驗證窗格中的相對最佳，不代表未來報酬。
""")
